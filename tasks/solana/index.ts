...
export const deriveCustomKeys = (programIdStr: string, keyPairForLockbox: Keypair) => {
    const programId = publicKey(programIdStr);
    const eddsa: EddsaInterface = createWeb3JsEddsa(); // Optional, if eddsa is still used elsewhere
    const oftDeriver = new OftPDA(programId);

    // Use the provided lockbox keypair
    const lockBox = keyPairForLockbox;
    const escrowPK = lockBox.publicKey;
    const [oftStorePda] = oftDeriver.oftStore(escrowPK);
    ...
    return {
        ...
        escrowPK,
        oftStorePda,
        eddsa, // Keep this if it's required elsewhere in your logic
    };
};
