...
import { addComputeUnitInstructions, deriveConnection, deriveKeys, deriveCustomKeys, getExplorerTxLink, output } from './index'
...
task('lz:oft:solana:create', 'Mints new SPL Token and creates new OFT Store account')
    ...
    .setAction(
        async ({
 	    ...
 	    /* Additional logic to generate SPL token address of choice in matching format as eddsa.generate... */
 	    
 	    const keyPairForToken = loadKeypairFromFile('./keypairs/splKeypair.json')
	    const keyPairForLockbox = loadKeypairFromFile('./keypairs/lockboxKeypair.json')  

	    // Convert the publicKey from Uint8Array to a base58 string
	    const publicKeyStringSPL = new PublicKey(keyPairForToken.publicKey).toString()
	    const publicKeyStringLockBox = new PublicKey(keyPairForLockbox.publicKey).toString()

	    // Return the full keypair object with publicKey as a string
	    const fullKeypairSPL = {
	      publicKey: publicKeyStringSPL,
	      secretKey: keyPairForToken.secretKey
	    }
	    
	    const fullKeypairLockbox = {
	    ...
	    */
            ...
            const mint = isMABA
               ...
               : createSignerFromKeypair(umi, fullKeypairSPL)
	    ...
	    console.log("Entering lockbox")
	    
            const lockboxSigner = createSignerFromKeypair({ eddsa: eddsa }, lockBox) //createSignerFromKeypair(fullKeypairLockbox, lockBox) //createSignerFromKeypair({ eddsa: eddsa }, lockBox) //createSignerFromKeypair(umi, fullKeypairLockbox) 
            ...
            console.log("OFT init build complete")
            ...
            console.log("Compute unit instructions complete")
