...
cp <insert_endpoint_keypair_file_name.json> ./target/deploy/endpoint-keypair.json --force

cp <insert_oft_keypair_file_name.json> ./target/deploy/oft-keypair.json --force

anchor keys sync
```

### If you created vanity keypairs, skip this step.
...
### Print the keys to terminal in order to view the public keypairs ( and copy the oft address to the lib.rs as follows )
